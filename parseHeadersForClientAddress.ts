...
export function parseHeadersForClientAddress(
  ...
  // Forwarding headers (X-Forwarded-For / X-Forwarded-Port / X-Real-IP) are
  // attacker-controllable on a direct connection. Only honor them when the
  // deployment has explicitly opted in to trusting an upstream proxy;
  // otherwise return nothing so callers fall back to the real socket address.
  if (!trustProxy) {
    return { ip, port };
  }
